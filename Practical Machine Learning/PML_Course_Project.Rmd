...
if(file.exists("RF_ModelFit.RData")){
    load("RF_ModelFit.RData")
}else{
    print("The Model has not yet been generated. This may take awhile. Estimated time 1.5 hrs")
    set.seed(1337)
    ModelFit <- train(classe~., method="rf", data=Trimmed_Training_Setv2,
                      preprocess=c("center", "scale"))
}

Training_Pred <- predict(ModelFit, newdata=Trimmed_Training_Setv2)

confusionMatrix(Training_Pred, Trimmed_Training_Setv2$classe)
```

# Summary
A random forest model was able to get 100% in-sample accuracy. For many datasets this would indicate a
high probably of overfitting, however one of the other [HAR Dataset][1] did give an accuracy of 99.41%.
This would suggest that the model may be an excellent fit for this dataset. The model was then applied
to the 20 samples within the test set.
